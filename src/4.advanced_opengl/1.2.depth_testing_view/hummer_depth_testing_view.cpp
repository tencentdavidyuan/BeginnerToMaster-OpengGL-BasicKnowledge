...
int main() {
	...
	GLFWwindow* _window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"4_1_2_hummer_depth_testing_view", NULL, NULL);
	if (_window == NULL) {
	...
	// -------------------------------------
	HummerShader _depthShader("4_1_2_DepthShader.vs",
		"4_1_2_DepthShader.fs");
	...
	unsigned int _specularMap;
	{
		...
		_depthShader.use();
		glUniform1i(glGetUniformLocation(_depthShader.ID, "material.diffuse"), 0);
		glUniform1i(glGetUniformLocation(_depthShader.ID, "material.specular"), 1);
	}
	...
	unsigned int _planeMap;
	{
		...
		_depthShader.use();
		glUniform1i(glGetUniformLocation(_depthShader.ID, "planeMap"), 0);
	}
	...
	while (!glfwWindowShouldClose(_window))
	{
		...
		_depthShader.use();
		_depthShader.setVec3("light.direction", _lightDir);
		_depthShader.setVec3("viewPos", _viewPos);

		_depthShader.setMat4("projection", _projection);
		_depthShader.setMat4("view", _view);
		...
		for (unsigned int i = 0; i < 2; ++i)
		{
			...
			_depthShader.setMat4("model", _model);
